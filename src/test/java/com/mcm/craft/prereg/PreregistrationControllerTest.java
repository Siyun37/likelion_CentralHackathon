...
class PreregistrationControllerTest extends ApiTestSupport {
    ...
    @BeforeEach
    void setUpProduct() {
        // data.sql이 기동 시 심어둔 시드 상품 중 하나를 사용
    ...
    private String createCustomer() {
        return customerRepository.save(Customer.create("cus_test_" + UUID.randomUUID())).getId();
    }

    private String requestJson(String customerId, Long productId, String name, String phone, Boolean consent) {
        return """
                {
                  "customer_id": "%s",
                  "product_id": %s,
